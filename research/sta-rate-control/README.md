...
ISTA-OPT02已完成诊断和局部搜索：发现物理离地前nu提前积分、lambda2放大初始条件敏感性的风险机制；8次新分轴干预及56次局部搜索均完成，但两名入围候选仍未达性能门槛，未运行独立验证、不改默认参数/保护。见[报告与曲线](reports/ISTA_OPT02.md)、[协议](ista_opt02/PROTOCOL_CN.md)、[运行说明](ista_opt02/RUN_CN.md)。没有把局部成功起降称为实机安全证明或ISTA总体优胜。

ISTA-OPT01专项搜索已结束：12套候选、46次尝试（25接受/21倾角超限），未找到满足目标的参数；唯一完整候选C01的六场景RMSE均不优于固定ESTA，按预注册停止，未运行验证/留出、不更换默认参数。见 [结果报告](reports/ISTA_OPT01.md)、[协议](ista_opt01/PROTOCOL_CN.md) 和 [运行说明](ista_opt01/RUN_CN.md)。这是固定ESTA、额外ISTA调参预算的后续研究，不属于等预算B组，不覆盖原M10结论。
...
已完成 M00～M09 的各阶段限定范围验收。M09新增 `MC_RTC_DIV=1/2/4`：同固件三算法九格起降/低幅激励通过，实测250/125/62.5Hz，传感器回调保持250Hz；当时90个C++/57个Python用例通过。见 [M09报告](reports/M09.md) 和 [分频运行/PlotJuggler说明](m09/RUN_CN.md)。默认仍PID/DIV1；M09完成当时尚未执行M10。ISTA/DIV4的pitch RMSE约为自身DIV1的2.23倍，功能通过不等于性能改善。原motor_limits仍丢样；M09命令TV/频谱使用完整的实际更新诊断序列，不冒充原motor话题无损。
