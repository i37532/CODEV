...
from design_rb import HERE,REPO,jobs,load
from summary_rb import summarize
I05=HERE.parent/'i05'
def main():
 ...
 head=subprocess.check_output(['git','rev-parse','HEAD'],cwd=REPO,text=True).strip();fp=HERE/'FROZEN_RB.json'
 if subprocess.check_output(['git','log','-1','--format=%H','--',str(fp.relative_to(REPO))],cwd=REPO,text=True).strip()!=head:raise RuntimeError('Run only frozen R-B HEAD')
 ...
 for n,job in enumerate(manifest):
  label=f"{n:04d}_I05RB_{job['algorithm']}_m{job['mode']}_s{job['seed']}";run=root/label;jf=root/(label+'.job.json');jf.write_text(json.dumps(job,indent=2)+'\n');env['M10_JOB']=str(jf);cmds=[];print('START',n+1,'/',len(manifest),label,flush=True);started=time.time()
  ...
  ap=run/'i05_analysis.json';row=json.loads(ap.read_text()) if ap.exists() else dict(success=False,algorithm=job['algorithm'],seed=job['seed'],failure_class='infrastructure',error='No analysis');now=(row.get('source_head'),row.get('binary_sha256'));anchor=now if anchor is None else anchor
