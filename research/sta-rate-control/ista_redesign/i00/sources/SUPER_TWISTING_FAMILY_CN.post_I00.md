...
> **2026-09-18 I00 勘误：下面三条保留为原始整理文字，第二条不得用作一般受扰精度保证。** 原 PDF Definition 1 的集合要求 virtual_s=0 且 nu=0；Lemma 4 的 h² 描述以持续留在该集合内为条件，未证明受扰时该集合的可达性/不变性。原 ISTA 在恒定扰动 d≠0 下存在 s=h*d、nu=-d 的平衡点，不满足上述条件。完整推导、生产内核回归及固定文献版本见 [I00 勘误](ERRATA_I00_CN.md)。历史实验与原文快照保留。
...
> **I00 补充限定：** 上表“大步长表现通常优于 ESTA”仅保留为历史概括，不能解释为真实受扰跟踪误差或起降安全必然更优；应分别报告偏差、波动、RMSE、控制负担和失败率。参见同一勘误。
