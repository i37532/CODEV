...
struct StaAxesApplication {
	static bool ready(bool iris_sitl, const StaProtection::Config &c)
	{
		...
		// I05R-B admits the independently calibrated full-axis configuration while
		// retaining the established I03 lifecycle.
		const bool proper_ready = c.mode == 3 && (c.axes == 1 || c.axes == 3 || c.axes == 7) && !c.takeoff.enabled;
